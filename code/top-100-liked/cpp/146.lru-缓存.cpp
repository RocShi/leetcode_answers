...
public:
  LRUCache(int capacity) { cache_cap_ = capacity; }

  int get(int key) {
    auto cache_map_iter = cache_map_.find(key);
    if (cache_map_iter != cache_map_.end()) {
      auto cache_iter = cache_map_iter->second;
      cache_.splice(cache_.begin(), cache_, cache_iter);
  ...
  void put(int key, int value) {
    ...
    if (cache_map_iter != cache_map_.end()) {
      ...
      cache_map_.insert({key, cache_.begin()});
    }
  }
};
